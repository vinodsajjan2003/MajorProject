...
THREAT_CATEGORIES = {
    'Malware': ['malware', 'virus', 'worm', 'infection', 'trojan', 'backdoor', 'rootkit', 'botnet', 'spyware'],
    'Ransomware': ['ransom', 'encrypt', 'decrypt', 'bitcoin', 'payment', 'locked', 'files', 'crypto'],
    'Phishing': ['phish', 'credential', 'password', 'login', 'bank', 'account', 'email', 'authenticate'],
    'Carding': ['card', 'credit', 'cvv', 'dump', 'fullz', 'bank', 'visa', 'mastercard', 'paypal'],
    'Exploit': ['exploit', 'vulnerability', 'cve', 'zero-day', 'buffer', 'overflow', 'injection', 'attack'],
    'Fraud': ['fraud', 'fake', 'counterfeit', 'scam', 'scheme', 'money', 'laundering', 'identity theft'],
    'Hacking Services': ['hack', 'crack', 'bruteforce', 'access', 'server', 'database', 'account', 'password'],
    'DDoS': ['ddos', 'denial of service', 'attack', 'botnet', 'flood', 'traffic', 'bandwidth'],
    'SQL Injection': ['sql', 'injection', 'query', 'database', 'information_schema', 'union', 'select'],
    'Trojan': ['trojan', 'backdoor', 'remote access', 'control', 'rat', 'keylogger', 'spy'],
    'Spyware': ['spyware', 'surveillance', 'monitor', 'keylogger', 'tracking', 'activity'],
    'Scam': ['scam', 'scheme', 'trick', 'deceive', 'fake', 'false', 'misleading']
}
...
def train_threat_model(input_file, model_dir='models'):
    ...
    try:
        ...
        if train_df.shape[0] == 0:
            ...
            return {
                'success': False,
                'error': 'No labeled data available for training'
            }
        ...
        labeled_data_path = os.path.join(model_dir, 'labeled_data.csv')
